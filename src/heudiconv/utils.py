...
def load_heuristic(heuristic: str) -> ModuleType:
    ...
    if os.path.sep in heuristic or os.path.lexists(heuristic):
        ...
        # Try to import the module name exactly as specified
        try:
...
def serialize_heuristic(mod: ModuleType) -> str:
    ...
    For single-file heuristics, returns the entire file content.
    For module-based heuristics, attempts to get the complete source.
    ...
    -------
    str
        Python code as a string containing the complete heuristic
        
    Raises
    ------
    RuntimeError
        If the heuristic source cannot be safely extracted
    """
    import inspect
    
    # For single-file heuristics, just read and return the entire file
    if hasattr(mod, '__file__') and mod.__file__:
        heuristic_file = mod.__file__.rstrip('co')  # Remove .pyc/.pyo extension if present
        
        # Check if this is NOT marked as a module-based heuristic
        if not getattr(mod, '_is_module_heuristic', False):
            try:
                with open(heuristic_file, 'r') as f:
                    return f.read()
            except (OSError, IOError) as e:
                raise RuntimeError(
                    f"Failed to read heuristic file {heuristic_file}: {e}"
                )
    
    # For module-based heuristics, try to get the complete source
    try:
        source = inspect.getsource(mod)
        lgr.info("Successfully extracted source for module-based heuristic")
        return source
    except (OSError, TypeError) as e:
        lgr.error(
            "Failed to extract source for module-based heuristic: %s", e
        )
        
        # Last resort: try to read __init__.py if it exists
        if hasattr(mod, '__file__') and mod.__file__:
            init_file = mod.__file__.rstrip('co')
            if op.exists(init_file):
                try:
                    with open(init_file, 'r') as f:
                        content = f.read()
                    lgr.warning(
                        "Using __init__.py content only - this may not include "
        ...
        raise RuntimeError(
            f"Cannot serialize heuristic module {mod.__name__}. "
